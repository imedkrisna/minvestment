...
cpi<-cpi|>filter(Economy%in%ec)|>filter(Year>=1990)|>filter(Year<=2022)
...
xr<-xr|>select(Year,Economy,`National currency at current prices`)|>filter(Economy%in%ec)|>filter(Year>=1990)|>filter(Year<=2022)
...
fdi<-fdi|>select(Year,Economy,`US$ at current prices in millions`)|>filter(Economy%in%ec)|>filter(Year>=1990)|>filter(Year<=2022)
#fdi<-fdi[nchar(fdi$Economy)<=3,]
fdi<-fdi|>rename(fdi=`US$ at current prices in millions`)

gdp<-read_csv('data/US_GDPTotal.csv')
gdp<-gdp|>select(Year,Economy,`US$ at current prices in millions`)|>filter(Economy%in%ec)|>filter(Year>=1990)|>filter(Year<=2022)
#gdp<-gdp[nchar(gdp$Economy)<=3,]
gdp<-gdp|>rename(gdp=`US$ at current prices in millions`)

pop<-read_csv('data/US_PopTotal.csv')
pop<-pop|>select(Year,Economy,`Absolute value in thousands`)|>filter(Economy%in%ec)|>filter(Year>=1990)|>filter(Year<=2022)
#pop<-pop[nchar(pop$Economy)<=3,]
pop<-pop|>rename(pop=`Absolute value in thousands`)

sin<-inner_join(cpi,fdi,by=c("Year","Economy"))
sin<-inner_join(sin,xr,by=c("Year","Economy"))
sin<-inner_join(sin,gdp,by=c("Year","Economy"))
sin<-inner_join(sin,pop,by=c("Year","Economy"))

sin<-sin|>mutate(Economy=replace(Economy,Economy=="960","360"))|>na.omit()
...
sin_out<-sin|>
  synthetic_control(outcome = fdi,
                    ...
                    i_time=2009,
                    generate_placebos = T)|>
  generate_predictor(time_window=2002:2009,
  ...
  generate_predictor(time_window=2009,
                     fdi3=fdi)|>
  generate_weights(optimization_window = 1990:2009,
